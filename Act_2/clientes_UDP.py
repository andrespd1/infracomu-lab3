...
# Thread function
def clientOperation(clientSocket, id, numClientes, serverAddress):
    received_hash = ''
    ...
    clientSocket.sendto(b'ping', serverAddress)
    ...
    print('[CLIENT {0}]: Integrity of data is:'.format(clientSocket.getsockname()), received_hash == computedHash,'\n-------------------------------------')
...
def MainClientThread(numClientes):
    ...
    for x in range(1, numClientes + 1):
        ...
        # Create the client
        cliente = Thread(name = x, target = clientOperation, args = (clientSocket, x, numClientes, serverAddress))
